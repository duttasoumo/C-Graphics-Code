...
void suther(int x1,int y1,int x2,int y2);
...
{
int xx1,yy1,xx2,yy2;
int px1,py1,px2,py2,px3,py3,px4,py4,ppx1,ppy1,ppx2,ppy2;
int flag1,flag2,flag3,flag4;
float m;/*slope of the intersecting line*/
int b1,b2;
char a[80];
b1=bit_code(x1,y1);
b2=bit_code(x2,y2);
...
/*sutherland cohen algo starts*/
m=(float)(y2-y1)/(x2-x1);
flag1=flag2=flag3=flag4=1;
/*case1 : to find intersection at x=xL*/
yy1=m*(xL-x1)+y1;
	if(yy1<yB || yy1>yT)
	{
	px1=xL;
...
/*case2 : to find intersection at x=xR*/
yy1=m*(xR-x1)+y1;
	if(yy1<yB || yy1>yT)
	{
	px2=xR;
...
/*case3 : to find intersection at y=yB*/
xx1=x1+1.0/m*(yB-y1);
	if(xx1<xL || xx1>xR)
	{
	px3=xx1;
...
/*case4 : to find intersection at y=yT*/
xx1=x1+1.0/m*(yT-y1);
	if(xx1<xL || xx1>xR)
	{
	px4=xx1;
...
/*to test whether the intersection points are valid or not*/
	if(flag1==0 && flag2==0 && flag3==0 && flag4==0)
	{
	outtextxy(250,30,"**No valid intersection point.*8");
	getch();
	return;
	}
	if(flag1==1 && flag2==1)
